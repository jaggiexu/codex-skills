...
每次生成后将优先级分布写入 `generation-history.json`。下次生成同类模块时：
- 对比历史分布，若偏差 >10% 则在 Phase 2.8 提示
- 用户手动调整过的分布记入 `user-preferences.json`，后续自动应用

### 风险等级积累

- 某模块多次生成后，历史缺陷数据累积形成风险画像
- 高频修正的模块自动提升风险等级
- 用户标记的"历史缺陷多"模块入库，影响后续优先级判断
